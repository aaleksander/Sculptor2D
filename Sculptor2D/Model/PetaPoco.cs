...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
		public Database()
		{
			throw new Exception("Необходимо перееопределить конструктор по умолчанию");
		}
...
*/
			...
			return rxParams.Replace(_sql, m =>
			{
				...
				if (int.TryParse(param, out paramIndex))
				{
					...
					if (paramIndex < 0 || paramIndex >= args_src.Length)
						throw new ArgumentOutOfRangeException(string.Format(CultureInfo.InvariantCulture, "Parameter '@{0}' specified but only {1} parameters supplied (in `{2}`)", paramIndex, args_src.Length, _sql));
				...
				else
				{
					...
					if (!found)
						throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Parameter '@{0}' specified but none of the passed arguments have a property with this name (in '{1}')", param, _sql));
				}
					...
					(arg_val as byte[]) == null)
				{
					...
					var tmp = arg_val as System.Collections.IEnumerable;
		...
		void AddParam(IDbCommand cmd, object item, string ParameterPrefix)
		{
			...
			if (idbParam != null)
			{
				idbParam.ParameterName = string.Format(CultureInfo.InvariantCulture, "{0}{1}", ParameterPrefix, cmd.Parameters.Count);
				cmd.Parameters.Add(idbParam);
				return;
			}

			var p = cmd.CreateParameter();
			p.ParameterName = string.Format(CultureInfo.InvariantCulture, "{0}{1}", ParameterPrefix, cmd.Parameters.Count);
			if (item == null)
			{
			...
			try
			{
				...
				try
				{
					using (var cmd = CreateCommand(_sharedConnection, sql, args))
					{
						...
						OnExecutedCommand(cmd);
						return (T)Convert.ChangeType(val, typeof(T), CultureInfo.CurrentCulture);
					}
				}
				finally
				{
		...
		string AddSelectClause<T>(string sql)
		{
			...
			if (!rxSelect.IsMatch(sql))
			{
				...
				if (!rxFrom.IsMatch(sql))
					sql = string.Format(CultureInfo.InvariantCulture, "SELECT {0} FROM {1} {2}", cols, tableName, sql);
				else
					sql = string.Format(CultureInfo.InvariantCulture, "SELECT {0} {1}", cols, sql);
			}
			return sql;
		}
		...
		public IList<T> Fetch<T>(Sql sql) 
		{
		...
		public void BuildPageQueries<T>(long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage) 
		{
			...
			if (_dbType == DBType.SqlServer || _dbType == DBType.Oracle)
			{
				...
				sqlPage = string.Format(CultureInfo.InvariantCulture, "SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
		...
		// Fetch a page	
		public Page<T> Page<T>(long aPage, long itemsPerPage, string sql, params object[] args) 
		{
			string sqlCount, sqlPage;
			BuildPageQueries<T>((aPage-1)*itemsPerPage, itemsPerPage, sql, ref args, out sqlCount, out sqlPage);
		...
		public Page<T> Page<T>(long aPage, long itemsPerPage, Sql sql) 
		{
			return Page<T>(aPage, itemsPerPage, sql.SQL, sql.Arguments);
		}


		public IList<T> Fetch<T>(long aPage, long itemsPerPage, string sql, params object[] args)
		{
			return SkipTake<T>((aPage - 1) * itemsPerPage, itemsPerPage, sql, args);
		}

		public IList<T> Fetch<T>(long aPage, long itemsPerPage, Sql sql)
		{
			return SkipTake<T>((aPage - 1) * itemsPerPage, itemsPerPage, sql.SQL, sql.Arguments);
		}

		public IList<T> SkipTake<T>(long skip, long take, string sql, params object[] args)
		{
			string sqlCount, sqlPage;
		...
		public IList<T> SkipTake<T>(long skip, long take, Sql sql)
		{
		...
		// Multi Fetch
		public IList<TRet> Fetch<T1, T2, TRet>(Func<T1, T2, TRet> cb, string sql, params object[] args) { return Query<T1, T2, TRet>(cb, sql, args).ToList(); }
		public IList<TRet> Fetch<T1, T2, T3, TRet>(Func<T1, T2, T3, TRet> cb, string sql, params object[] args) { return Query<T1, T2, T3, TRet>(cb, sql, args).ToList(); }
		public IList<TRet> Fetch<T1, T2, T3, T4, TRet>(Func<T1, T2, T3, T4, TRet> cb, string sql, params object[] args) { return Query<T1, T2, T3, T4, TRet>(cb, sql, args).ToList(); }
		...
		public IList<TRet> Fetch<T1, T2, TRet>(Func<T1, T2, TRet> cb, Sql sql) { return Query<T1, T2, TRet>(cb, sql.SQL, sql.Arguments).ToList(); }
		public IList<TRet> Fetch<T1, T2, T3, TRet>(Func<T1, T2, T3, TRet> cb, Sql sql) { return Query<T1, T2, T3, TRet>(cb, sql.SQL, sql.Arguments).ToList(); }
		public IList<TRet> Fetch<T1, T2, T3, T4, TRet>(Func<T1, T2, T3, T4, TRet> cb, Sql sql) { return Query<T1, T2, T3, T4, TRet>(cb, sql.SQL, sql.Arguments).ToList(); }
		...
		// Multi Fetch (Simple)
		public IList<T1> Fetch<T1, T2>(string sql, params object[] args) { return Query<T1, T2>(sql, args).ToList(); }
		public IList<T1> Fetch<T1, T2, T3>(string sql, params object[] args) { return Query<T1, T2, T3>(sql, args).ToList(); }
		public IList<T1> Fetch<T1, T2, T3, T4>(string sql, params object[] args) { return Query<T1, T2, T3, T4>(sql, args).ToList(); }
		...
		public IList<T1> Fetch<T1, T2>(Sql sql) { return Query<T1, T2>(sql.SQL, sql.Arguments).ToList(); }
		public IList<T1> Fetch<T1, T2, T3>(Sql sql) { return Query<T1, T2, T3>(sql.SQL, sql.Arguments).ToList(); }
		public IList<T1> Fetch<T1, T2, T3, T4>(Sql sql) { return Query<T1, T2, T3, T4>(sql.SQL, sql.Arguments).ToList(); }
				...
				for (int i = 1; i < types.Length; i++)
				{
					...
					for (int j = i - 1; j >= 0; j--)
					{
						...
						if (candidates.Count() > 1)
							throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "Can't auto join {0} as {1} has more than one property of type {0}", types[i], types[j]));
					...
					if (!handled)
						throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "Can't auto join {0}", types[i]));
				}
		...
		Delegate FindSplitPoint(Type typeThis, Type typeNext, string sql, IDataReader r, ref int pos)
		{
			...
			throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "Couldn't find split point between {0} and {1}", typeThis, typeNext));
		}
		...
		{
			return FirstOrDefault<T>(string.Format(CultureInfo.InvariantCulture, "WHERE {0}=@0", EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey)), primaryKey) != null;
		}
		public T Single<T>(object primaryKey) 
		{
			return Single<T>(string.Format(CultureInfo.InvariantCulture, "WHERE {0}=@0", EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey)), primaryKey);
		}
		public T SingleOrDefault<T>(object primaryKey) 
		{
			return SingleOrDefault<T>(string.Format(CultureInfo.InvariantCulture, "WHERE {0}=@0", EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey)), primaryKey);
		}
					...
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						foreach (var i in pd.Columns)
						{
							...
							if (autoIncrement && primaryKeyName != null && string.Compare(i.Key, primaryKeyName, true)==0)
							{
								...
								{
									names.Add(i.Key);
									values.Add(string.Format(CultureInfo.InvariantCulture, "{0}.nextval", pd.TableInfo.SequenceName));
								}
								continue;
							}

							names.Add(EscapeSqlIdentifier(i.Key));
							values.Add(string.Format(CultureInfo.InvariantCulture, "{0}{1}", _paramPrefix, index++));
							AddParam(cmd, i.Value.GetValue(poco), _paramPrefix);
						}

						cmd.CommandText = string.Format(CultureInfo.InvariantCulture, "INSERT INTO {0} ({1}) VALUES ({2})",
						...
						switch (_dbType)
						{
							...
							case DBType.PostgreSQL:
								...
								{
									cmd.CommandText += string.Format(CultureInfo.InvariantCulture, "returning {0} as NewID", EscapeSqlIdentifier(primaryKeyName));
									DoPreExecute(cmd);
									id = cmd.ExecuteScalar();
								}
								else
							...
							case DBType.Oracle:
								if (primaryKeyName != null)
								{
									cmd.CommandText += string.Format(CultureInfo.InvariantCulture, " returning {0} into :newid", EscapeSqlIdentifier(primaryKeyName));
					...
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						if (columns == null)
						{
							foreach (var i in pd.Columns)
							{
								...
								if (string.Compare(i.Key, primaryKeyName, true, CultureInfo.InvariantCulture) == 0)
								{
		...
		{
			var pd = PocoData.ForType(typeof(T));
			return Execute(string.Format(CultureInfo.InvariantCulture, "UPDATE {0} {1}", EscapeTableName(pd.TableInfo.TableName), sql), args);
		}

		public int Update<T>(Sql sql)
		{
			var pd = PocoData.ForType(typeof(T));
			return Execute(new Sql(string.Format(CultureInfo.InvariantCulture, "UPDATE {0}", EscapeTableName(pd.TableInfo.TableName))).Append(sql));
		}
		...
		public int Delete(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
		{
			...
			// Do it
			var sql = string.Format(CultureInfo.InvariantCulture, "DELETE FROM {0} WHERE {1}=@0", EscapeTableName(tableName), EscapeSqlIdentifier(primaryKeyName));
		...
		{
			var pd = PocoData.ForType(typeof(T));
			return Execute(string.Format(CultureInfo.InvariantCulture, "DELETE FROM {0} {1}", EscapeTableName(pd.TableInfo.TableName), sql), args);
		}

		public int Delete<T>(Sql sql)
		{
			var pd = PocoData.ForType(typeof(T));
			return Execute(new Sql(string.Format(CultureInfo.InvariantCulture, "DELETE FROM {0}", EscapeTableName(pd.TableInfo.TableName))).Append(sql));
		}
		...
		public bool IsNew(string primaryKeyName, object poco)
		{
			...
			else
			{
				...
				if (pi == null)
					throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "The object doesn't have a property matching the primary key column name '{0}'", primaryKeyName));
		...
		public class PocoData
		{
			...
			public PocoData(Type t)
			{
				...
				TableInfo.PrimaryKey = a.Length == 0 ? "RN" : (a[0] as PrimaryKeyAttribute).Value;
